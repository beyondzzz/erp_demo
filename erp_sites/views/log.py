...
def paging(request, ONE_PAGE_OF_DATA, condition, selectType, departmentID):
    ...
    if curPage == 1:
        if condition == None:
            basicObjs = Log.objects.all()[0:ONE_PAGE_OF_DATA]
        else:
            if 'timeFrom' in selectType and 'timeTo' in selectType:
                timeFrom = selectType['timeFrom']
                timeTo = selectType['timeTo']
                if len(personIDList) > 0:
                    basicObjs = Log.objects.filter(Q(operator_identifier__in=personIDList) & Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))[0:ONE_PAGE_OF_DATA]
                else:
                    basicObjs = Log.objects.filter(Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))[0:ONE_PAGE_OF_DATA]
            else:
                if len(personIDList) > 0:
                    basicObjs = Log.objects.filter(Q(operator_identifier__in=personIDList) & Q(**condition))[0:ONE_PAGE_OF_DATA]
                else:
                    basicObjs = Log.objects.filter(**condition)[0:ONE_PAGE_OF_DATA]
