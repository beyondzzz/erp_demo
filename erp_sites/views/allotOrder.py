...
def paging(request, ONE_PAGE_OF_DATA, condition, selectType):
    ...
    if curPage == 1:
        if condition == None:
            basicObjs = AllotOrder.objects.all()[0:ONE_PAGE_OF_DATA]
        else:
            if 'timeFrom' in selectType and 'timeTo' in selectType:
                timeFrom = selectType['timeFrom']
                timeTo = selectType['timeTo']
                basicObjs = AllotOrder.objects.filter(Q(**condition) & Q(allot_date__gte=timeFrom) & Q(allot_date__lte=timeTo))[0:ONE_PAGE_OF_DATA]
            else:
                basicObjs = AllotOrder.objects.filter(**condition)[0:ONE_PAGE_OF_DATA]
