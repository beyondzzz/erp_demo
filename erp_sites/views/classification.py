...
def firstClassificationDelete(request):
    try:
        if isTokenExpired(request):
            ...
            json2Dict = json.loads(request.body)
            identifierLists = json2Dict['identifiers']
            for identifierList in identifierLists:
                identifier = int(identifierList['identifier'])
                classifications = Classification.objects.filter(id=identifier,parent_id=0)
...
def paging(request,ONE_PAGE_OF_DATA,condition,isFirst,selectType):
    logRecord = basic_log.Logger('record')
    pagingSelect = {}
    datasJSON = []
    ...
    if isFirst:
        if condition == None:
            basicsCount = Classification.objects.filter(is_delete=0,parent_id=0).count()
        else:
...
def conditionSelect(condition, isFirst, selectType):
    conditionSelect = {}
    datasJSON = []
    if isFirst:
        if 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' not in selectType:
        ...
        if 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' not in selectType:
            timeFrom = selectType['timeFrom']
            timeTo = selectType['timeTo']
            basicObjs = Classification.objects.filter(Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))
        elif 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' in selectType:
            timeFrom = selectType['timeFrom']
            timeTo = selectType['timeTo']
            operator = selectType['operator']
            ...
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicObjs = Classification.objects.filter(Q(operator_identifier__in=personIdentifierList) & Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))
        elif 'timeFrom' not in selectType and 'timeTo' not in selectType and 'operator' in selectType:
            operator = selectType['operator']
            persons = Person.objects.filter(name__icontains=operator)
            personIdentifierList = []
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicObjs = Classification.objects.filter(Q(operator_identifier__in=personIdentifierList) & Q(**condition))
        else:
            basicObjs = Classification.objects.filter(**condition)
        for basicObj in basicObjs:
            basicObjJSON = getClassification(basicObj)
            childrenJSON = []
            children = Classification.objects.filter(parent_id=basicObj.id, is_delete=0)
        ...
        if 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' not in selectType:
            timeFrom = selectType['timeFrom']
            timeTo = selectType['timeTo']
            basicsCount = Classification.objects.filter(Q(parent_id__gt=0) & Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo)).count()
        elif 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' in selectType:
            timeFrom = selectType['timeFrom']
            timeTo = selectType['timeTo']
            operator = selectType['operator']
            ...
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicsCount = Classification.objects.filter(Q(parent_id__gt=0) & Q(operator_identifier__in=personIdentifierList) & Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo)).count()
        elif 'timeFrom' not in selectType and 'timeTo' not in selectType and 'operator' in selectType:
            operator = selectType['operator']
            persons = Person.objects.filter(name__icontains=operator)
            personIdentifierList = []
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicsCount = Classification.objects.filter(Q(parent_id__gt=0) & Q(operator_identifier__in=personIdentifierList) & Q(**condition)).count()
        else:
            basicsCount = Classification.objects.filter(Q(parent_id__gt=0) & Q(**condition)).count()
        if 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' not in selectType:
                timeFrom = selectType['timeFrom']
                timeTo = selectType['timeTo']
                basicObjs = Classification.objects.filter(Q(parent_id__gt=0) & Q(**condition) & Q(operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))
        elif 'timeFrom' in selectType and 'timeTo' in selectType and 'operator' in selectType:
            timeFrom = selectType['timeFrom']
            timeTo = selectType['timeTo']
            operator = selectType['operator']
            ...
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicObjs = Classification.objects.filter(Q(parent_id__gt=0) & Q(operator_identifier__in=personIdentifierList) & Q( **condition) & Q( operator_time__gte=timeFrom) & Q(operator_time__lte=timeTo))
        elif 'timeFrom' not in selectType and 'timeTo' not in selectType and 'operator' in selectType:
            operator = selectType['operator']
            persons = Person.objects.filter(name__icontains=operator)
            personIdentifierList = []
            for person in persons:
                personIdentifierList.append(person.identifier)
            basicObjs = Classification.objects.filter(Q(parent_id__gt=0) &Q(operator_identifier__in=personIdentifierList) & Q(**condition))
        else:
            basicObjs = Classification.objects.filter(Q(parent_id__gt=0) & Q(**condition))
        for basicObj in basicObjs:
            basicObjJSON = getClassification(basicObj)
            datasJSON.append(basicObjJSON)
    conditionSelect['code'] = 200
    dataJSON = {}
    dataJSON['total'] = basicsCount
    dataJSON['datas'] = datasJSON
    conditionSelect['data'] = dataJSON
