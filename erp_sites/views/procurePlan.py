...
def paging(request, ONE_PAGE_OF_DATA, condition, selectType):
    ...
    if curPage == 1:
        if condition == None:
            basicObjs = ProcureTable.objects.filter(is_delete=0)[0:ONE_PAGE_OF_DATA]
        else:
            if 'timeFrom' in selectType and 'timeTo' in selectType:
                timeFrom = selectType['timeFrom']
                timeTo = selectType['timeTo']
                basicObjs = ProcureTable.objects.filter(
                    Q(**condition) & Q(generate_date__gte=timeFrom) & Q(generate_date__lte=timeTo))[
                            0:ONE_PAGE_OF_DATA]
            else:
                basicObjs = ProcureTable.objects.filter(**condition)[0:ONE_PAGE_OF_DATA]
